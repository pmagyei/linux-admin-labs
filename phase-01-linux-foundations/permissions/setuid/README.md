...
setuid script = generally ignored on Linux, the kernel ignores the setuid bit on a interpreted script at runtime. Because setuid was not honoured on the interpreted script, the process kept the caller’s normal effective UID. It only functions on compiled binary executables.
